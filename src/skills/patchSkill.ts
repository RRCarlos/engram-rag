...
export function patchSkill(content: string, agentId: AgentId): PatchResult {
  ...
  if (existingMatch !== null) {
    ...
    const replaced =
      content.slice(0, start) + fresh + content.slice(end);
    ...
    if (replaced === content) {
      return { content, changed: false, reason: "block already up to date" };
    }
    if (replaced !== content.slice(0, start) + idempotent + content.slice(end)) {
      return {
        content,
        changed: false,
